...
class _CaregiverAssignmentDetailsScreenState
  ...
  Map<String, dynamic>? get _selectedCaregiver =>
      _selectedCaregiverId != null
          ? widget.availableCaregivers.firstWhere(
            (c) => c['user_id'] == _selectedCaregiverId,
            orElse: () => {},
          )
          : null;
  ...
  Map<String, dynamic>? get _selectedCareplan =>
      _selectedCareplanId != null
          ? widget.availableCarePlans.firstWhere(
            (p) => p['careplan_id'] == _selectedCareplanId,
            orElse: () => {},
          )
          : null;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final assignments = await _dbHelper.getCaregiverAssignments(
        widget.userId,
      );
      ...
      if (payments.isNotEmpty) {
        ...
        payments.sort((a, b) {
          final aDate =
              a['paid_at'] != null
                  ? DateTime.parse(a['paid_at'])
                  : DateTime(1970);
          final bDate =
              b['paid_at'] != null
                  ? DateTime.parse(b['paid_at'])
                  : DateTime(1970);
      ...
      if (assignments.isNotEmpty) {
        ...
        final activeAssignment =
            assignments.where((a) => a['status'] == 'Active').toList();
  ...
  Future<void> _saveAssignment() async {
    ...
    try {
      ...
      final selectedPlan = await _dbHelper.getCareplanById(
        _selectedCareplanId!,
      );
      if (selectedPlan != null) {
        // Create a transaction ID if needed
        final tempTransactionId =
            'assignment_${widget.userId}_${DateTime.now().millisecondsSinceEpoch}';

        if (_existingPayment != null) {
          // Update existing payment - use 'Paid' instead of 'Active'
          await _dbHelper.updatePayment(_existingPayment!['payment_id'], {
            'careplan_id': _selectedCareplanId,
            'amount': selectedPlan['monthly_rate'],
            'status':
                'Paid', // Changed from 'Active' to 'Paid' to match constraint
          });
        } else {
      ...
      if (_existingAssignment != null) {
        ...
        await _dbHelper
            .updateCaregiverAssignment(_existingAssignment!['assignment_id'], {
